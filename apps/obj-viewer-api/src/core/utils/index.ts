...
import { formatString, getExtensionFromName } from '@app/common/utils';
...
export const mapData = (template: Record<string, any>, dataset: Record<string, any>) => {
  if (!template) return null;
  const mappedData = {
    ...template,
  };
  Object.keys(mappedData).forEach((key) => {
    const mapField = mappedData[key];
    if (mapField.type && mapField.type.toLowerCase() === 'mapping' && dataset.hasOwnProperty(mapField.value)) {
      mappedData[key] = dataset[mapField.value];
    } else {
      mappedData[key] = mapField;
    }
  });
  return mappedData;
};
